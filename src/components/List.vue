<template>
  ...
  <div class="fileList" @dragover.prevent @drop="dropFile">
    <el-table stripe :data="ossList" ref="fileList" v-loading="loading" :max-height="tableHeight" style="width: 100%"
      ...
      <el-table-column label="操作" width="240" header-align="right">
        <div class="ace-operation" slot-scope="scope">
          <div @click="getUrl(scope.row.url)" v-if="!scope.row.dir" type="text"
                     style="font-size: 15px; font-weight: 700; color: #409eff; cursor: pointer">获取地址
          </div>
          <div @click="downloadClick(scope.row)" v-if="!scope.row.dir" type="text"
                     style="font-size: 15px; font-weight: 700; color: #409eff; cursor: pointer">下载
          </div>
          <div type="text" @click="renameClick(scope.row)" style="font-size: 15px; font-weight: 700; color: #888; cursor: pointer">
            重命名
          </div>
          <div @click="deleteKeyTool(scope.row)" type="text"
                     style="font-size: 15px; font-weight: 700; color: #f56c6c; cursor: pointer">删除
          </div>
        </div>
      </el-table-column>
    </el-table>
    <el-dialog title="重命名" :visible.sync="renameVisible" width="400px" :before-close="handleClose" append-to-body
    ...
    <el-dialog title="图片预览" :visible.sync="imgPreviewVisible" width="800px" :before-close="handleClose"
...
export default {
  ...
  methods: {
    ...
    async fileNameClick(row) {
      // if (this.$store.state.path=='' || row.name=='corrosion-detection'){
      //   return
      // }
